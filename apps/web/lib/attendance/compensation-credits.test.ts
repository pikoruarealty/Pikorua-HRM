...
describe("allocateCompensationCredits", () => {
  test("matches a single leave day against a single covering credit", () => {
    ...
    expect(redemptions).toEqual([
      { creditId: "credit-1", date: d("2026-07-10"), kind: "unpaid_leave", requestId: "req-1" },
    ]);
  });
  ...
  test("leave date exactly on earnedDate or expiresAt is redeemed (inclusive bounds)", () => {
    ...
    ).toEqual([{ creditId: "credit-1", date: d("2026-07-01"), kind: "unpaid_leave", requestId: "req-1" }]);
    expect(
      allocateCompensationCredits([], [{ date: d("2026-08-30"), requestId: "req-1" }], [credit]),
    ).toEqual([{ creditId: "credit-1", date: d("2026-08-30"), kind: "unpaid_leave", requestId: "req-1" }]);
  });
  ...
  test("each credit is used at most once, even with multiple eligible leave days", () => {
    ...
    expect(redemptions).toEqual([
      { creditId: "credit-1", date: d("2026-07-05"), kind: "unpaid_leave", requestId: "req-1" },
    ]);
  });
  ...
  test("prefers the soonest-expiring covering credit, leaving the longer-lived one available", () => {
    ...
    expect(redemptions).toEqual([
      { creditId: "expires-soon", date: d("2026-07-10"), kind: "unpaid_leave", requestId: "req-1" },
    ]);
  });
  ...
  test("absence days are matched before unpaid-leave days, even when the leave day is older", () => {
    const redemptions = allocateCompensationCredits(
      [{ date: d("2026-07-20") }],
      [{ date: d("2026-07-05"), requestId: "req-1" }],
      [{ id: "credit-1", earnedDate: d("2026-07-01"), expiresAt: d("2026-08-30") }],
    ...
    expect(redemptions).toEqual([
      { creditId: "credit-1", date: d("2026-07-20"), kind: "absence", requestId: null },
    ]);
  });

  test("leftover credits after absences are exhausted still cover unpaid-leave days", () => {
    const redemptions = allocateCompensationCredits(
      [{ date: d("2026-07-05") }],
      [{ date: d("2026-07-10"), requestId: "req-1" }],
      [
    ...
    expect(redemptions).toHaveLength(2);
    const byKind = new Map(redemptions.map((r) => [r.kind, r]));
    expect(byKind.get("absence")?.date).toEqual(d("2026-07-05"));
    expect(byKind.get("unpaid_leave")?.date).toEqual(d("2026-07-10"));
  });

  test("an absent day earned via a credit that came in advance is covered even without a request", () => {
    // "someone has came in advance and then not came in that month" — the
    // credit was earned before the absence, well within the 60-day window.
    const redemptions = allocateCompensationCredits(
      [{ date: d("2026-07-15") }],
      [],
      [{ id: "credit-1", earnedDate: d("2026-07-01"), expiresAt: d("2026-08-30") }],
    ...
    expect(redemptions).toEqual([
      { creditId: "credit-1", date: d("2026-07-15"), kind: "absence", requestId: null },
    ]);
  });
});
