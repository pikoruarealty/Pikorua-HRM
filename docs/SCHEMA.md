...
| components | jsonb NULL | Added 2026-08-08 (Pillar 6). The breakdown behind a monthly composite score: `{ score, components: [{ key, label, weight, nominalWeight, value, detail, description }], unavailable: [key], inert: [key] }`. `weight` is the **renormalised** share (always totals 100 across the listed components); `value` is that component's own 0–100 result. **Null on `weekly` rows and on rows computed before Pillar 6** — consumers must handle null and fall back to showing the bare score. `description` and `inert` were added 2026-08-11 and are absent on older rows; readers fall back to `COMPONENT_DESCRIPTIONS` and to treating `salesOutcome` as the only inert slot. A component with no data for the period is *unavailable*, not zero — it is dropped and the remaining weights are renormalised, so a first-month hire with no review yet is not penalised for it. `inert` is the subset of `unavailable` skipped only because this department's profile weights it 0, so the UI doesn't explain it as "we couldn't measure it" |
| | | **Weights are department-aware as of 2026-08-11** (`lib/performance/composite.ts`). **Tech** — output 50, quality 17, attendance 17, timeliness 8, commitments-kept 8, `salesOutcome` 0. **Sales** — the same 50-point output block split across the owner's locked 3:5:7 ratio using two slots: `output` carries daily-call attainment (10) and `salesOutcome` carries site visits + bookings blended 17:23 (40); then quality 20, attendance 20, commitments-kept 10, and `timeliness` **0** (due dates are a Tech concept, and hitting target inside the month is already what the paced attainment measures). The `ComponentKey` set is unchanged in both — it is the contract every downstream reader keys off |
...
`self_logged_cap_percent` (default 30) is applied in `gatherMonthlyInputs()`, which fetches the self-logged slice of the point ledger alongside the total and runs both through `cappedSelfLoggedPoints()` from `lib/work/adhoc.ts`. The capped figure is what the **Output** component is built from, and the overflow is reported as `selfLoggedExcluded` (surfaced in the breakdown's `detail` string, e.g. `"70 of 120 pts (30 over self-logged cap)"`). The cap is a share of the employee's *total* credited points for the month, applied at **scoring** time and never at approval — refusing work that genuinely happened would be a lie about the work, and the Lead would have no way to act on it. `monthlyPointSplit()` does the same split for a single employee, for per-employee screens.
