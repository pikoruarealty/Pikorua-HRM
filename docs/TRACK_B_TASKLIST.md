...
**Implementation note (fixed 2026-07-14):** `POST /sub-units/:id/work-items` and `PATCH /work-items/:id` only validated that `assignedTo` referenced *some* employee — a Lead could assign a task to any employee org-wide, including other teams' members and non-Employee roles (e.g. HR). Fixed by adding a team-membership check for the Lead path in both routes: when the actor is the owning Lead (not Admin/HR), `assignedTo` must belong to a `Team` where `teamLeadId === session.employeeId`, else `422 VALIDATION`. Mirrored (UX only, not the security boundary) in the test-UI's "New WorkItem" dropdown so Leads only see their own team's members as options. Also fixed a related access bug: `GET /work-items/mine` was gated with a strict `isEmployeeRole` whitelist that excluded Leads entirely, returning `403 FORBIDDEN` even though Leads have Employee records and can be WorkItem assignees like anyone else — broadened the gate to `isEmployeeRole(role) || isLeadRole(role)`. Live-verified: a Tech Lead's attempt to assign a task to an HR user now 422s; assigning to their own team's member succeeds; the Tech Lead's `GET /work-items/mine` now returns their own assigned tasks instead of 403.
...
**Implementation note (fixed 2026-07-14):** Same root cause as 1.2's `work-items/mine` fix — `POST /daily-selections` used the strict `isEmployeeRole` whitelist and 403'd Leads trying to select their own tasks for the day; broadened to `isEmployeeRole(role) || isLeadRole(role)`. Separately, `GET /daily-selections/today`'s Lead branch scoped visible employees via `team.members` (`TeamMembers` rows), but a Lead's own Employee record isn't guaranteed to be a member row on the team they lead (`Team.teamLeadId` and `Employee.teamId` are independent fields) — so a Lead could be scoped out of their own selections. Fixed by explicitly adding `session.employeeId` to `scopedEmployeeIds` if not already present. Live-verified: a Tech Lead can now POST a daily selection for a task assigned to themselves, and `GET /daily-selections/today` includes it.
...
**Storage revised 2026-07-14 — S3 replaced with local disk:** deployment target is a single GCP VM (not serverless), so `lib/storage/s3.ts` and the `@aws-sdk/*` deps were removed in favor of `lib/storage/local.ts`. POST now takes real `multipart/form-data` (`docType` + `file` fields, 10MB cap) instead of a client-supplied URL — file bytes are written under `apps/web/uploads/documents/<employeeId>/<uuid>.<ext>` (gitignored). The `fileUrl` DB column now stores an opaque internal storage key rather than a public URL; GET rewrites it to `/api/v1/employees/:id/documents/:docId/file`, a new route that re-checks the same self-or-Finance auth on every read and streams the bytes back — nothing under `uploads/` is directly web-servable. `bun run build` clean. Live-verified: multipart upload succeeded and the file was readable on local disk; download as the owning employee returned the exact file bytes; download as an unrelated employee 403'd; a pre-existing seeded document (whose `fileUrl` predates this change and points at a fake S3 URL) 404'd gracefully instead of crashing the route.
